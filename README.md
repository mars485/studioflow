...
Вход и регистрация ограничены общими для API-процессов счётчиками в PostgreSQL: 20 попыток на email и 100 на адрес подключения за 15 минут, затем 429. В production-конфигурации Caddy передаёт API адрес клиента; за внешним прокси задайте точный `TRUSTED_PROXY` (см. DEPLOYMENT.md). В dev-конфигурации адрес может быть общим для запросов через Vite. HTTPS обязателен для публичной установки: задайте `SESSION_COOKIE_SECURE=true`, точные `ALLOWED_ORIGINS` и проксируйте `/api` с того же домена. В dev Docker порты доступны только на `127.0.0.1`. `SECRET_KEY`/`ACCESS_TOKEN_EXPIRE_MINUTES` остались для совместимости конфигурации и не используются для непрозрачных сессий. `DEV_AUTH_ENABLED` больше никогда не обходит вход.

## Запуск через Docker

Для VPS используется отдельная [production-конфигурация](DEPLOYMENT.md). Команды ниже предназначены для локальной разработки.
...
8. Автоматические резервные копии во внешнее хранилище, мониторинг и регулярная проверка восстановления.
